...
def logout():
    ...
    if username and supabase:
        # Optionally remove from database on logout, or let it expire
        try:
...
def update_status():
    ...
    if supabase:
        try:
            # Check if user exists in DB first to avoid repeated heavy stats fetch
            # In a real app, you'd store stats less frequently
